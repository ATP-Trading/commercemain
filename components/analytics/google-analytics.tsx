...
export function GoogleAnalytics() {
  ...
  useEffect(() => {
    // Ask again for Google advertising; previous choices covered only Meta ads.
    try { setShow(!localStorage.getItem(CONSENT_KEY) || !localStorage.getItem(MARKETING_CONSENT_KEY) || !localStorage.getItem(GOOGLE_ADS_CONSENT_KEY)); } catch { /* No optional storage. */ }
  ...
  if (!show) return null;
  const choose = (analytics: boolean, marketing: boolean) => { setAnalyticsConsent(analytics, marketing); setMarketingConsent(marketing); setShow(false); };
  return <section aria-label={ar ? 'تفضيلات التحليلات' : 'Analytics preferences'} dir={ar ? 'rtl' : 'ltr'} className="fixed bottom-20 md:bottom-4 inset-x-3 z-[70] mx-auto max-w-xl rounded-2xl border border-neutral-700 bg-neutral-950 p-4 text-white shadow-xl">
    <p className="text-sm leading-6">{ar ? 'نستخدم Google Analytics لفهم استخدام الموقع، وGoogle وMeta لقياس الإعلانات وتخصيصها. اختر ملفات تعريف الارتباط التي تسمح بها. يمكنك تغيير اختيارك من تفضيلات التحليلات.' : 'We use Google Analytics to understand site usage, and Google and Meta to measure and personalise ads. Choose which cookies to allow. You can change your choice in Analytics preferences.'}</p>
