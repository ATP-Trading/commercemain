...
export function AccountShell({ title, path, children }: { title: string; path: string; children: ReactNode }) {
  ...
  return <main dir={ar ? 'rtl' : 'ltr'} className="min-h-screen bg-neutral-950 px-4 pb-28 pt-6 sm:py-8 text-base leading-relaxed text-white [&_input]:min-h-12 [&_input]:text-base [&_label]:text-base [&_button]:min-h-12 [&_button]:text-base"><div className="mx-auto max-w-4xl space-y-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-4"><h1 className="text-3xl font-semibold">{title}</h1>{customer && path === '/account' && <Button variant="outline" className="border-neutral-600 bg-neutral-900 text-white hover:bg-neutral-800 hover:text-white" onClick={() => logout(`/${locale}`)}>{ar ? 'تسجيل الخروج' : 'Sign out'}</Button>}</div>
    {customer && path === '/account' && <div className="space-y-1 text-neutral-300"><p className="text-lg">{[customer.firstName, customer.lastName].filter(Boolean).join(' ') || (ar ? 'أهلًا بك' : 'Welcome')}</p><p dir="ltr" className={`break-words ${ar ? 'text-right' : 'text-left'}`}>{customer.email}</p></div>}
