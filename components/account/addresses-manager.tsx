...
function AddressForm({ address, isDefault, onCancel, onSaved }: { address?: Address; isDefault: boolean; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={save} className="space-y-5 rounded-xl border border-atp-gold bg-neutral-900 p-5">
    ...
    <fieldset disabled={saving} className="grid gap-4 sm:grid-cols-2">
      ...
      <Button type="submit" className="bg-atp-gold text-black hover:bg-atp-gold/90">{saving ? (ar ? 'جارٍ الحفظ…' : 'Saving…') : (ar ? 'حفظ العنوان' : 'Save address')}</Button><Button type="button" variant="outline" className="border-neutral-600 bg-neutral-900 text-white hover:bg-neutral-800 hover:text-white" onClick={onCancel}>{ar ? 'إلغاء' : 'Cancel'}</Button>
    </fieldset>
