...
function AddressForm({ address, isDefault, onCancel, onSaved }: { address?: Address; isDefault: boolean; onCancel: () => void; onSaved: () => void }) {
  ...
  const fields = [
    ...
    ['zip', ar ? 'الرمز البريدي (اختياري)' : 'Postal code (optional)', 'postal-code', false], ['phoneNumber', ar ? 'رقم التواصل مع رمز الدولة (اختياري)' : 'Contact number with country code (optional)', 'tel', false],
  ] as const
  return <form onSubmit={save} className="space-y-5 rounded-xl border border-atp-gold bg-neutral-900 p-5">
    ...
    <fieldset disabled={saving} className="grid gap-4 sm:grid-cols-2">
      {fields.map(([key, label, autoComplete, required]) => <div key={key} className="space-y-2"><Label htmlFor={`address-${key}`}>{label}</Label><Input id={`address-${key}`} name={key} autoComplete={autoComplete} defaultValue={address?.[key] || ''} required={required} maxLength={key === 'phoneNumber' ? 30 : key === 'firstName' || key === 'city' ? 100 : 255} type={key === 'phoneNumber' ? 'tel' : 'text'} dir={key === 'phoneNumber' ? 'ltr' : undefined} placeholder={key === 'phoneNumber' ? '+971501234567' : undefined} pattern={key === 'phoneNumber' ? '\\+[1-9][0-9 ()-]{6,24}' : undefined} /></div>)}
      <div className="space-y-2"><Label htmlFor="address-zone">{ar ? 'الإمارة / المنطقة' : 'Emirate / region'}</Label>{country === 'AE' ? <select id="address-zone" name="zoneCode" required defaultValue={address?.zoneCode || ''} className="h-11 w-full rounded-md border border-neutral-600 bg-neutral-900 px-3"><option value="">{ar ? 'اختر الإمارة' : 'Choose an emirate'}</option>{emirates.map(([code, arabic, english]) => <option key={code} value={code}>{ar ? arabic : english}</option>)}</select> : <Input id="address-zone" name="zoneCode" defaultValue={address?.zoneCode || ''} />}</div>
