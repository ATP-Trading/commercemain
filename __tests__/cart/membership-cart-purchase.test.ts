...
for(const existing of [false,true]) it(`sends the allocated plan when ${existing?'adding to':'creating'} a cart`,async()=>{
 ...
 const requests:any[]=[]
 const cart={id:'saved-cart',checkoutUrl:'https://example.com/checkout',lines:{edges:[]},cost:{totalAmount:{amount:'99',currencyCode:'AED'},subtotalAmount:{amount:'99',currencyCode:'AED'}},totalQuantity:1}
 vi.stubGlobal('fetch',vi.fn(async(_url,options)=>{
  const request=JSON.parse(options.body);requests.push(request)
  const data=request.query.includes('query VariantPlan')?{node:{product:{requiresSellingPlan:true},sellingPlanAllocations:{nodes:[{sellingPlan:{id:'gid://shopify/SellingPlan/123',name:'Annual'}}]}}}:existing?{cartLinesAdd:{cart,userErrors:[]}}:{cartCreate:{cart,userErrors:[]}}
 ...
 expect(existing?requests[1].variables.lines:requests[1].variables.input.lines).toEqual(expected)
 expect(requests[1].variables.language).toBe('AR')
